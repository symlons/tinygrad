...
class UOp(RandMixin, metaclass=UOpMetaClass):
  ...
  def to_elf(self) -> TinyELF:
    ...
    return TinyELF(self.src[3].arg, self.arg.function_name, self.arg.target, sig, profile_key=self.key, dynamic_smem=self.arg.dynamic_smem)
...
class ProgramInfo:
  ...
  def from_sink(sink:UOp, target:Target=Target()) -> ProgramInfo:
    ...
    return ProgramInfo(sink.arg.name if isinstance(sink.arg, KernelInfo) else "test", tuple(global_size),
                       ...
                       sink.arg.dynamic_smem if isinstance(sink.arg, KernelInfo) else 0)
